...
//
// Examples:
//   node buddy-reroll.js --species dragon --rarity legendary --shiny
//   node buddy-reroll.js --species dragon --min-stats 80
//   node buddy-reroll.js --check f17c2742a00b2345c22fddc830959a6847ceb561fa06adb26b74b1a91ac657bc
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--min-stats': {
        const next = args[i + 1];
        opts.minStatsVal =
          next && !next.startsWith('--') ? parseInt(args[++i]) : 90;
        break;
      }
      case '--max':
        opts.max = parseInt(args[++i]);
        break;
      case '--count':
...
console.log(
  `Runtime: ${runtimeLabel}${isBun ? '' : ' (results will NOT match Claude Code)'}`,
);
console.log(
  `Searching: ${filters.join(', ') || 'any'} (max ${opts.max.toLocaleString()}, find ${opts.count})`,
);
console.log('');
...
for (let i = 0; i < opts.max; i++) {
  ...
  if (
    opts.minStatsVal &&
    !Object.values(r.stats).every((v) => v >= opts.minStatsVal)
  )
    continue;
  ...
  console.log(
    `#${found} [${r.rarity}] ${r.species} eye=${r.eye} hat=${r.hat} shiny=${r.shiny}`,
  );
...
if (found === 0) {
  console.log(
    `No match found in ${opts.max.toLocaleString()} iterations (${elapsed}s)`,
  );
} else {
